...
export const ORDINARY_EQ_HANDOFF_ENTRY_STATUS =
	"EQ Port 행 · 포인터는 청록색 CENTER에서 같은 직선의 다른 슬롯까지 놓지 않고 드래그 · 키보드는 흰 테두리 1 시작에서 Enter";
...
function buildReturnEqHandoff(pitchMillimeters: number): OrdinaryNextPortHandoffPresentation {
	...
	return Object.freeze({
		...
		action: "start-eq",
		label: "다음 · EQ Port 행",
		instruction: `PITCH ${pitchMeters} m 준비 · 청록색 CENTER에서 드래그`,
		ariaLabel: `다음 작업: 현재 EQ PITCH ${pitchMeters} m로 완성할 수 있는 직선에서 청록색 CENTER 슬롯을 시작해 같은 직선의 다른 슬롯까지 놓지 않고 드래그하여 EQ Port 행을 만듭니다. 키보드는 진입 후 표시되는 흰 테두리 1번 시작에서 Enter를 사용합니다`,
		description: `현재 EQ PITCH ${pitchMeters} m로 EQ Port 행을 완성할 수 있는 직선 레일이 준비되었습니다. 청록색 CENTER에서 같은 직선의 다른 슬롯까지 놓지 않고 드래그하거나, 흰 테두리 1번 시작에서 Enter로 시작합니다.`,
	});
}
