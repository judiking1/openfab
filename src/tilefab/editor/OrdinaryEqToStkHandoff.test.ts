...
describe("ordinaryEqToStkHandoff", () => {
	it("offers one optional STK handoff while repeated EQ placement remains available", () => {
		expect(ordinaryEqToStkHandoff(READY_CONTEXT)).toEqual({
			...
			description:
				"현재 포트 구성과 요구 개수를 확인하고 금색 ◇를 선택한 뒤 Stocker 생성을 누르세요.",
		});
		expect(ORDINARY_STK_HANDOFF_ENTRY_STATUS).toContain("현재 포트 구성의 요구 개수");
		expect(ORDINARY_STK_HANDOFF_ENTRY_STATUS).toContain("선택한 뒤 Stocker 생성");
		expect(ORDINARY_STK_HANDOFF_ENTRY_STATUS).toContain("Stocker 생성");
	});
