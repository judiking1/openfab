...
describe("ordinaryFirstPortHandoff", () => {
	it("routes the Rail commit message from the actual legal OHB slot count", () => {
		...
		expect(ORDINARY_RAIL_COMMIT_PENDING_STATUS).not.toContain("Port는 장비 메뉴");
		...
		expect(ordinaryRailCommitStatus(0)).not.toContain("Port는 장비 메뉴");
		expect(ordinaryRailCommitStatus(1)).toContain("Port는 장비 메뉴");
	...
	it("offers one task-first handoff after an ordinary rail exposes OHB slots", () => {
		expect(ordinaryFirstPortHandoff(READY_CONTEXT)).toEqual({
			...
			ariaLabel:
				"다음 작업: 장비 메뉴에서 레일 옆 하늘색 원을 클릭하거나 Enter로 OHB Port 1개 놓기",
			description:
