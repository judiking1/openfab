...
const outputDirectory = path.join(
  repositoryRoot,
  "src/tilefab/generated/synthetic-fab-presets",
);
const presetSpecifications = Object.freeze([
  Object.freeze({
    ...
    fileName: "paired-circulation-fab-52.default.v2.json",
    label: "Paired-Circulation Production FAB",
  }),
  Object.freeze({
    requestId: "full-fab-52",
    artifactId: "full-fab-52.default.v1",
    fileName: "full-fab-52.default.v1.json",
    label: "Full Production FAB",
  }),
  Object.freeze({
    requestId: "large-fab-60",
    artifactId: "large-fab-60.default.v1",
    fileName: "large-fab-60.default.v1.json",
    label: "Legacy Large FAB",
  }),
  Object.freeze({
    requestId: "parallel-hall-fab-12",
    artifactId: "parallel-hall-fab-12.default.v1",
    fileName: "parallel-hall-fab-12.default.v1.json",
    label: "Parallel Hall FAB",
  }),
  Object.freeze({
    requestId: "central-spine-fab-24",
    artifactId: "central-spine-fab-24.default.v1",
    fileName: "central-spine-fab-24.default.v1.json",
    label: "Central Spine FAB",
  }),
  Object.freeze({
    requestId: "production-fab-60",
    artifactId: "production-fab-60.default.v1",
    fileName: "production-fab-60.default.v1.json",
...
try {
  const starterModule = await vite.ssrLoadModule(
    "/src/tilefab/compile/SyntheticFabStarter.ts",
  );
  ...
  const unexpectedArtifactFiles = (await readdir(outputDirectory))
    .filter(
      (fileName) =>
        fileName.endsWith(".json") && !expectedArtifactFiles.has(fileName),
    )
    .sort();
  ...
  for (const specification of presetSpecifications) {
    const request = starterModule.defaultSyntheticFabStarterRequest(
    ...
    if (artifact.artifactId !== specification.artifactId) {
      throw new Error(
        `${specification.label} generated an unexpected artifact identity.`,
      );
    }

    const firstRoundTrip =
      artifactModule.hydrateSyntheticFabStarterCertifiedArtifact(
        artifact,
        request,
      );
    ...
    if (
      ...
      throw new Error(
        `${specification.label} certified payload must omit preview geometry.`,
      );
    }
...
function assertHydratedIdentity(label, expected, actual) {
  if (
    ...
    throw new Error(
      `${label} artifact failed materialization identity verification.`,
    );
  }
}
...
function assertBufferIsolation(label, first, second) {
  ...
  for (const [viewPath, firstView] of firstViews) {
    ...
    if (
      ...
      throw new Error(
        `${label} does not isolate typed-array buffer ${viewPath}.`,
      );
    }
  }
}
